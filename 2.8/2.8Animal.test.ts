...
class Fish extends Animal {
   ...
   constructor(name: string, food: string, saltwater: boolean) {
      super(name, '', food);  // Fish do not have a sound, so we pass an empty string
...
describe("Testing animals", () => {
   ...
   describe("Fish", () => {
      ...
      it("doesn't make sound", () => {
         ...
         expect(goldfish.soundOff()).toBe("The goldfish is a fish and does not make sounds.");
      });
   ...
   describe("Birds", () => {
      ...
      it("make sounds", () => {
         ...
         expect(swallow.soundOff()).toBe('The swallow makes the sound "chattering chirp".');
      });

      it("might be able to fly", () => {
         // Bird have a new method using the fly speed to give us these strings
         expect(swallow.fly()).toBe("The swallow flies at speeds of up to 11 meters per second!");
